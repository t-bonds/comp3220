...
(define while
  (lambda (look run myEnv)
    (cond
      ((exp myEnv look)
       (begin (interp (car run) myEnv)
              (set! myEnv(interp (cadr run) myEnv))
              (while look run myEnv))))))
...
(define exp
  (lambda (myEnv e)
    (cond
      ...
      ((eqv? (car e) '<) (< (exp myEnv (cadr e))(exp myEnv (caddr e))))
      ((eqv? (car e) '>) (> (exp myEnv (cadr e))(exp myEnv (caddr e))))
      ((eqv? (car e) 'and) (and (exp myEnv (cadr e))(exp myEnv (caddr e)))); what if the operands were reversed?
...
(define gprog
  '(program
    (= x 10)
    (while (> x 0) ((print x)(= x (- x 1)))))) ; should print numbers 10 - 1


; Call the interpreter for xprog
(interpreter fprog)
